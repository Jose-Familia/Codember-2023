function contarPalabras(mensaje: string): string {
  ...
  const palabraContador: { [palabra: string]: number } = {};
  const resultado: string[] = [];

  for (const palabra of palabras) {
    if (palabra in palabraContador) {
      palabraContador[palabra]++;
    } else {
      palabraContador[palabra] = 1;
    }
  }

  for (const palabra in palabraContador) {
    resultado.push(palabra + palabraContador[palabra]);
  }
